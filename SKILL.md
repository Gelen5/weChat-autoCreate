...
- [ ] Step 3: 配图 —— 内联 SVG 信息图 / HTML 截图 / gpt-image-2 生图
...
3. **gpt-image-2 生图**：插画、封面、meme 等非数据类配图。`imagegen.ts` 已封装，默认模型 `gpt-image-2`。

正文图单张 ≤1MB（微信 `media/uploadimg` 限制），超了用 `sips` 压缩转 jpg。
